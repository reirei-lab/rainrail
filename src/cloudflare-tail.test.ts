...
describe('Cloudflare tail source', () => {
  ...
  it('redacts tail URLs and exception strings before publishing while keeping stack frames', async () => {
    ...
    expect(JSON.stringify(event.payload)).not.toContain('tail-brace-token-secret');
    expect(JSON.stringify(event.payload)).not.toContain('tail-bracket-token-secret');
    expect(JSON.stringify(event.payload)).not.toContain('tail-paren-session-secret');
    expect(JSON.stringify(event.payload)).not.toContain('tail-bracket-structured-secret');
    expect(JSON.stringify(event.payload)).not.toContain('tail-paren-structured-secret');
