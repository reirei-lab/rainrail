...
| `reirei-harness/test/dashboard.test.ts` | Render and serve dashboard state, logs, settings, resume/reset, and termination actions. | `src/http-app.ts`, `src/operational-store.ts`, `src/agent-timeline.ts` | `src/dashboard-api.test.ts`, `src/operational-store.test.ts`, `src/agent-timeline.test.ts` | Covered by split. Rainrail documents operational state separately in `docs/operational-state.md`. |
