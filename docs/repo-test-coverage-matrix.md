...
| `github-eep-bridge/test/bridge-room.test.js` | BridgeRoom stores published events and replays them to SSE subscribers. | `src/event-delivery/bridge-room.ts`, `src/event-delivery/event-bus.ts` | `src/bridge-room.test.ts`, `src/event-bus.test.ts`, `src/manual-chat.test.ts` | Ported and expanded. Rainrail also covers duplicate ids, storage ordering, abort cleanup, replay limits, serialization failure handling, sanitized GitHub/Cloudflare/manual/chat delivery reference allowlists, context-scoped manual/chat replay payload normalization, and bounded manual/chat strings and attachments. |
