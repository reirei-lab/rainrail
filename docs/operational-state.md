...
`JsonFileOperationalStore` と `JsonFileOperationalStoreOptions` は Node 20 互換の JSON
file-backed adapter として残す。`rainrail setup --yes` が既存 workspace に補完する
既定 local store もこの JSON backend を使い、`databasePath` は
`.rainrail/operational.json` である。この path は local operational state を含むため、
setup は補完時に同じ workspace の `.gitignore` へ `.rainrail/operational.json` を追加する。
local Node server の config では top-level `operationalStore` で backend を選ぶ。
setup が補完する local dashboard API の例:
...
`databasePath: "${RAINRAIL_OPERATIONAL_DB}"` を渡す。SQLite を明示する場合の推奨
local path は `var/rainrail-operational.sqlite` である。focused unit test や embedding
test では `kind: "memory"` を使う。
...
JSON の場合は shared `JsonFileOperationalStore` と同じ object 形式を読み書きするため、
`events` は event id を key にした object、`dashboardLayout` は
`{ id: "user.dashboardLayout", items, updatedAt }` として保存する。local start の dashboard
events は process restart 後も local API で確認でき、`createRainrailNodeServer` が同じ
JSON file に保存した event も復元対象になる。SQLite の場合は shared `operational_events`
table を読み書きし、同じ DB に保存した event を復元対象にする。
