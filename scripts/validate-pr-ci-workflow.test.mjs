...
describe('pull request CI workflow', () => {
  ...
  it('publishes dashboard VRT comments from trusted workflow_run tooling', () => {
    ...
    expect(dashboardVrtCommentWorkflow).toContain('event_type="$(jq -r \'.workflow_run.event // ""\' "$GITHUB_EVENT_PATH")"');
    expect(dashboardVrtCommentWorkflow).toContain('pr_number="$(jq -r \'.workflow_run.pull_requests[0].number // ""\' "$GITHUB_EVENT_PATH")"');
    expect(dashboardVrtCommentWorkflow).toContain('pr_head_repo="$(jq -r \'.workflow_run.pull_requests[0].head.repo.full_name // ""\' "$GITHUB_EVENT_PATH")"');
    expect(dashboardVrtCommentWorkflow).toContain('Skipping dashboard VRT PR comment because workflow_run is not an in-repository pull request.');
    expect(dashboardVrtCommentWorkflow).toContain('if [ "$event_type" = "pull_request" ]');
    expect(dashboardVrtCommentWorkflow).toContain('$pr_head_repo" = "$GITHUB_REPOSITORY"');
    expect(dashboardVrtCommentWorkflow).toMatch(/^ {6}- name: Check out trusted tooling\n {8}if: \$\{\{ steps\.workflow-run-pr\.outputs\.eligible == 'true' \}\}\n {8}uses: actions\/checkout@v7\n {8}with:\n {10}fetch-depth: 1\n {10}persist-credentials: false$/m);
    expect(dashboardVrtCommentWorkflow).toMatch(/^ {6}- name: Verify workflow run is the latest PR head\n {8}if: \$\{\{ steps\.workflow-run-pr\.outputs\.eligible == 'true' \}\}\n {8}id: latest-pr-head\n {8}run: \|/m);
    expect(dashboardVrtCommentWorkflow).toContain('current_head_sha="$(gh api "repos/$GITHUB_REPOSITORY/pulls/${PR_NUMBER}" --jq \'.head.sha\')"');
    expect(dashboardVrtCommentWorkflow).toContain('if [ "$current_head_sha" = "${WORKFLOW_RUN_HEAD_SHA}" ]; then');
    ...
    expect(dashboardVrtCommentWorkflow).toContain('gh run download "${WORKFLOW_RUN_ID}" --repo "$GITHUB_REPOSITORY" --name dashboard-e2e-artifacts --dir dashboard-vrt-artifacts');
    ...
    expect(dashboardVrtCommentWorkflow).toContain('pre_publish_head_sha="$(gh api "repos/$GITHUB_REPOSITORY/pulls/${PR_NUMBER}" --jq \'.head.sha\')"');
    expect(dashboardVrtCommentWorkflow).toContain('if [ "$pre_publish_head_sha" != "${WORKFLOW_RUN_HEAD_SHA}" ]; then');
    ...
    expect(dashboardVrtCommentWorkflow).toContain('cml comment update --repo="$GITHUB_REPOSITORY" --target="pr/${PR_NUMBER}" --watermark-title="Rainrail dashboard VRT" dashboard-vrt-comment/vrt-comment.md');
    expect(dashboardVrtCommentWorkflow).toMatch(/^ {10}GH_TOKEN: \$\{\{ github\.token \}\}\n {10}REPO_TOKEN: \$\{\{ secrets\.CML_COMMENT_TOKEN \}\}\n {10}PR_NUMBER: \$\{\{ steps\.workflow-run-pr\.outputs\.pr_number \}\}\n {10}WORKFLOW_RUN_HEAD_SHA: \$\{\{ steps\.workflow-run-pr\.outputs\.head_sha \}\}$/m);
