...
describe('local dashboard start documentation', () => {
  it('documents the setup/start/open flow for a new local operator', () => {
    for (const required of [
      ...
      'For a Tailscale-only dashboard, bind to the machine',
      'If the same server should be reachable from both LAN and Tailscale',
      ...
      '`server.allowedHosts` must include the exact LAN IP',
      ...
      'Linux firewall',
      '`0.0.0.0` is never the browser URL',
      'Host header is outside the `server.allowedHosts`',
  ...
  it('keeps dashboard auth failure guidance and MVP exclusions explicit', () => {
    for (const required of [
      ...
      '`rainrail start` now auto-generates local dashboard bearer',
      ...
      'There is no localhost no-auth dashboard API mode for normal startup.',
